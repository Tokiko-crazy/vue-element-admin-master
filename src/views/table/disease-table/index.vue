  ...
  <el-card :body-style="{}" shadow="always" class="card">
    <div class="search" style="padding: 5px 0px">
      <div class="filter-container" style="margin-top:-10px;padding:0px;">
        ...
        <el-select v-model="listQuery.disease_type" placeholder="疾病类型" clearable style="width: 90px;margin-right: 10px;" class="filter-item">
    ...
    <el-table ref="multipleTable" :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 98%;" max-height=" 80%" @sort-change="sortChange" element-loading-text="拼命加载中" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="测序ID" width="150px" align="center">
        <template slot-scope="{row}">
          <div v-if="Array.isArray(row.sequences)">
            <p v-for="item in row.sequences">{{ item }}&nbsp&nbsp</p>
          </div>
          <span v-else>{{row.sequences}}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="被检测人" width="150px" align="center">
        <template slot-scope="{row}">
    ...
    <el-drawer title="样本信息详情" :with-header="false" :visible.sync="sample_information" direction="rtl" size="60%">
      ...
      <el-row>
        ...
        <disease-detail v-bind:current_id="current_id" :editAble="false" :deleteAble="true" />
      </el-row>
      <el-row>
        <el-col :span="24">
          <div class="" style="margin:20px 25px">样本信息</div>
        </el-col>
        <my-forms v-bind:current_id="sequences" :editAble="false" :deleteAble="false" />
      </el-row>
      <!-- </el-container> -->
    </el-drawer>
    <el-dialog :visible.sync="dialogExcelVisible">
      <upload-excel-component :on-success="handleSuccess" :before-upload="beforeUpload" />
      <!-- <el-progress :text-inside="true" :stroke-width="26" :percentage.sync="excelProcess"></el-progress> -->
    </el-dialog>
  </el-card>
</template>
...
export default {
  ...
  methods: {
    ...
    getOptions() {
      fetchDiseaseType().then((res) => {
        this.diseaseTypeOptions = res.data.map(function (obj) {
          return obj.name;
        });
      });
    },
    ...
    handleModifyStatus(row, status) {
      fetchArticleInfo({ id: row.id }).then((response) => {
        ...
        this.current_id = { diseease_id: [row.id] };
        console.log("传递给子组件", this.current_id, this.sequences);
    ...
    handleDownload() {
        ...
        this.downloadLoading = true;
        import("@/vendor/Export2Excel").then((excel) => {
          const tHeader = ["id","测序ID","疾病类型","被检测人","就诊年龄" ,"病理完整信息","进展","分型","分级","病理免疫组化","TNM","分期","亚型/部位" ,"术前肿瘤治疗情况（射频、TACE等）","主诉","饮酒（包括频度、种类、量）","高血压","糖尿病","吸烟史","既往肿瘤病史(若有，注明肿瘤类型)","系统疾病","家族史","抗病毒治疗","高血脂(TC、TG、LDL-C、HDL-C)","生化指标","淋巴细胞","中性粒细胞","术前AFP","术前CEA","术前CA19-9","HBV-DNA","乙肝表面抗原" ,"表面抗体" ,"e抗原","e抗体" ,"核心抗体","丙肝","HCV-Ab","HCV-RNA","甲状腺+","总胆红素","白蛋白","凝血酶原时间","肝性脑病","Child-Pugh","腹水","术后化疗","发现复发时间","靶向药物治疗（注明药物）","体内放疗（放射性粒子植入）","射频治疗","手术治疗","死亡时间","术后异常","存活时间" ,"术前肠镜","肠镜病理","肝转移" ,"个数","最大径（最大径和）（mm）" ,"大小（mm）","子灶" ,"肉眼癌栓" ,"癌栓位置" ,"包膜","坏死" ,"肝外侵犯（写明部位）","镜下包膜有无突破" ,"镜下包膜有无突破","镜下多灶性生长","镜下子灶" ,"微血管癌栓","肝硬化" ,"肝炎","BCLC","UICC","术后TACE（注明治疗次数）","术后抗病毒治疗（注明药物）" ,"PSA","Lauren分类","备注"];
          const filterVal = [
            "id",
            "title",
            "author",
