<template>
  <div id="appDis">
    ...
    <el-dialog :visible.sync="dialogFormVisible" title="病人信息" append-to-body="true" width="70%">
      <div class="filter-container" style="margin-top:-10px;margin-left:16px;padding:0px;">
        <el-button v-waves class="filter-item" type="primary" icon="el-icon-search" style="margin-right: 10px;" @click="handleFilter">
          搜索
        </el-button>
        <el-input v-model="listQuery.name" placeholder="姓名" style="width: 100px;margin-right: 10px;" class="filter-item" @keyup.enter.native="handleFilter" />
      ...
      <patient-detail ref="patientForm" v-bind:current_id="listQuery" :editAble="false" :deleteAble="false" @insertPatient="insertPatient" :chooseAble="true" :patientHeight="300" />
    </el-dialog>
  </div>
...
export default {
  props: ["dialogStatus", "dataForm","textMap"],
      ...
      rule: [
        ...
        },
        {
          ...
          props: {
            ...
            remote: true,
            autoComplete: "on",
            remoteMethod: this.getDiseseTpye,
            loading: this.loading,
            loadData: () => {},
            loadingText: "加载中...",
            noMatchText: "没有录入疾病类型",
            data: this.dsType,
            placeholder: "",
            // "collapse-tags": true,
          },
        ...
        },
        {
          ...
          validate: [
            { required: true, message: "请输入就诊年龄", trigger: "blur" },
          ],
        },
        {
          type: "input",
  ...
  mounted() {
    this.$nextTick(() => {
      this.getDiseseTpye();
      // this.IDoptions = this.dataForm.sequences.map(function (d) {
      //   return { value: d, label: d, disabled: false };
      // });
    });
  },
  methods: {
    ...
    insertPatient(val) {
      this.fApi.updateRule("patient", {
        value: val,
      });
      this.fApi.refresh(true);
    ...
    getDiseseTpye(query) {
      this.loading = true;
      const q = { type: query };
      fetchDiseaseType(query).then((res) => {
        this.loading = false;
        const diseaseType = res.data;
        this.dsType = diseaseType.map(this.optionChange);
        console.log("选项", this.dsType);
        this.fApi.updateRule("disease_type", {
          options: this.dsType,
        });
        this.fApi.refresh(true);
      });
