    ...
    readerData(rawFile) {
      ...
      return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
          ...
          console.log("workbook.Sheets:", startData);
          let sheetList = workbook.SheetNames;
          console.log("workbook.SheetNames:", sheetList);
          //存放json数组格式的表格数据
          let resultJson = [];
          sheetList.forEach((item) => {
            let worksheet = workbook.Sheets[item];
            let json = XLSX.utils.sheet_to_json(workbook.Sheets[item]);
            resultJson.push.apply(resultJson, json);
          });
          const firstSheetName = workbook.SheetNames[0];
          const worksheet_f = workbook.Sheets[firstSheetName];
          const header = this.getHeaderRow(worksheet_f);
          const results = resultJson;
          console.log("excle数据", resultJson);
